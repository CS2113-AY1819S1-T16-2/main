...
public class EditEventCommandParser implements Parser<EditEventCommand> {
    ...
    public EditEventCommand parse(String args) throws ParseException {
        ...
        ArgumentMultimap argMultimap =
                ArgumentTokenizer.tokenize(args, PREFIX_NAME, PREFIX_DESCRIPTION, PREFIX_START_DATE, PREFIX_END_DATE, PREFIX_LOCATION);
        ...
        if (argMultimap.getValue(PREFIX_START_DATE).isPresent()) {
            editEventDescriptor.setStartTime(ParserUtil.parseDate(argMultimap.getValue(PREFIX_START_DATE).get()));
        }
        if (argMultimap.getValue(PREFIX_END_DATE).isPresent()) {
            editEventDescriptor.setEndTime(ParserUtil.parseDate(argMultimap.getValue(PREFIX_END_DATE).get()));
        }
